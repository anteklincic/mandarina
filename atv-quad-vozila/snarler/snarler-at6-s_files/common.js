...
var cart = {
	'add': function(product_id, quantity) {
		$.ajax({
          ...
          // Google Tag Manager
          dataLayer.push({ 'event': 'addToCart', 'product.ID': product_id });
				}
			},
...
/*$(function() {
	var i = ( !!$("#livesearch").length ? $("#livesearch") : $("<ul id='livesearch'></ul>") ), s = $("#search [name=search]"), w = $("#search");
	function repositionLivesearch() {
    if ($(window).width() > 768) { i.show(); i.css({ top: (w.offset().top+w.outerHeight()), left: w.offset().left, width: w.outerWidth() }); } else { i.hide(); }
  }
